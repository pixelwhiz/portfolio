...
const App: React.FC = () => {
  ...
  return (
      <ChakraProvider>
        <Card zIndex={"100"} roundedTopEnd={"1rem"} roundedBottomEnd={"1rem"} rounded={"0"} backgroundColor={""} position={"fixed"} className={`bg-base-200 h-full w-64 fixed transition-all duration-200 ease-in-out transform ${isMenuOpen ? 'left-0' : '-left-64'}`}>
        ...
        <Box className={"hero bg-base-100 z-10"}>
          <SlideFade
             in={isOpen}
             offsetY={"1rem"}
             className={`grid relative overflow-x-hidden px-4 max-w-4xl pb-5 md:pb-0 sm:mt-14 gap-5 transition-all duration-200 ease-in-out transform ${
                 isMenuOpen ? "lg:left-64 xl:left-36 sm:left-72 left-64" : "left-0"
             }`}
          >
           {/* Header */}
           ...
           {/* Content */}
           <Card roundedBottom={"2rem"} roundedTopEnd={"2rem"} backgroundColor={"#00BD8DFF"} backgroundBlendMode={"color-dodge"} className={"shadow-sm shadow-base-content brightness-95 px-0 py-4 border-base-content/25 border"}>
             <Box className={"flex gap-3 ms-4 me-4 justify-center"}>
               <Box className="avatar my-auto">
                 <Avatar height={"6rem"} width={"6rem"} className="rounded-full" src={"https://avatars.githubusercontent.com/u/111522987?v=4"}>
                   {isFetchingLanyard ? (
                       <Card position={"absolute"} boxShadow={"none"} backgroundColor={"#00BD8DFF"} rounded={"full"} className={"h-7 w-7 ms-16 mt-16 absolute z-10 rounded-2xl"} >
           ...
           <Box className="grid md:flex gap-5" style={{ overflowX: 'hidden' }}>
             <Card rounded={"1rem"} backgroundColor={""} className="shadow-sm grid gap-2.5 w-full shadow-base-content p-4 lg:p-5 md:h-[20rem] border-base-content/25 bg-base-content/5 border">
               <label className="text-3xl text font-bold">Hello there!</label>
               <label className="font-normal brightness-150 text text-base-content">I'm a programmer who loves to code and learn
                 new things. I'm currently learning web development and I'm also interested in game development. I'm also a
                 huge fan of anime and manga. </label>
             ...
             <Card rounded={"1rem"} backgroundColor={""} className="shadow-sm justify-center shadow-base-content md:w-96 p-4 lg:p-5 md:h-[20rem] border-base-content/25 bg-base-content/5 border">
               <Box className={"grid gap-2.5 md:-translate-y-12"}>
